...
const PhishingWarningBanner: React.FC = () => {
  ...
  const warningText = t("please make sure you're visiting https://app.dexo.exchange - check the URL carefully.")
  const warningTextAsParts = warningText.split(/(https:\/\/app.Dexo Exchange.exchange)/g)
    ...
    <>
      ...
      {warningTextAsParts.map((text, i) => (
        <Text
          ...
          small
          as="span"
          bold={text === 'https://app.dexo.exchange'}
          color={text === 'https://app.dexo.exchange' ? '#FFFFFF' : '#BDC2C4'}
        >
          {text}
        </Text>
      ))}
